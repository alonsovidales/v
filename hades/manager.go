...
func GetHades(traders int, from int, collector charont.Int, unitsToUse, samplesToConsiderer, lastOpsToConsider, tradesThatCanPlay int) (hades *Hades) {
	hades = &Hades{
	...
	for i := 0; i < traders; i++ {
		for _, curr := range collector.GetCurrencies() {
			hades.traders[i] = hermes.GetWindowTrader(curr, int64(from+i)*1000000000, collector, unitsToUse, samplesToConsiderer)
		}
	}
